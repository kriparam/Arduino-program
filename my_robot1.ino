...
void setup() 
{
 ...
 t2=0;
 if(D!=0) { 
  //If ultrasonic sensor reads an obstruction in the vicinity of max distance then stop robot
 ...
 t1+=1; } }
  ...
  //If left IR sensor reads black line then move the robot left
 if(t2>=20) { 
  //If the robot is stuck while moving left then move robot back for 0.5 seconds and move robot left for 1 second.
 digitalWrite(5,LOW);
 digitalWrite(10,LOW);
 digitalWrite(6,HIGH); //left motor anticlockwise
 digitalWrite(9,HIGH); //right motor anticlockwise, moving backward when stuck
 ...
 t2+=1; } }
  ...
  //If the two IR sensor reads black line at both sides then the robot moves forward
 digitalWrite(5,HIGH); //left motor clockwise
 digitalWrite(10,HIGH); //right motor clockwise, moving forward
 digitalWrite(6,LOW);
 digitalWrite(9,LOW);
